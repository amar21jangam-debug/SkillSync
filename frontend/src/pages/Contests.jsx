...
const STATUS = {
  ...
  live: { label: "Live now", cls: "bg-[#FFFFFF] text-black animate-pulse" },
...
export default function Contests() {
  ...
  return (
    <div>
      <div className="mb-8">
        <div className="text-xs mono uppercase tracking-[0.25em] text-[#FFFFFF] mb-2">Contests</div>
            ...
            <div key={c.id} className="surface-card surface-card-hover p-6">
              ...
              <div className="grid grid-cols-3 gap-3 text-sm">
                <div>
                  ...
                  <div className="flex items-center gap-1 mt-1"><Clock size={13} className="text-[#FFFFFF]"/> {c.duration_min}m</div>
                </div>
                <div>
                  <div className="text-xs text-[#888] mono uppercase tracking-widest">Players</div>
                  <div className="flex items-center gap-1 mt-1"><Users size={13} className="text-[#FFFFFF]"/> {c.participants.toLocaleString()}</div>
                </div>
                <div>
                  <div className="text-xs text-[#888] mono uppercase tracking-widest">Prize</div>
                  <div className="flex items-center gap-1 mt-1 text-[#FFFFFF] text-xs"><Award size={13}/> {c.prize}</div>
                </div>
              </div>
      ...
      <div className="surface-card p-6">
        ...
        <div className="space-y-2">
          {data.leaderboard.map((u) => (
            <div key={u.rank} className={`flex items-center justify-between px-4 py-3 rounded-lg ${u.rank <= 3 ? "bg-[#FFFFFF]/10 border border-[#FFFFFF]/40" : "bg-[#141414]"}`}>
              <div className="flex items-center gap-4">
                <div className={`mono font-bold w-8 text-center ${u.rank === 1 ? "text-[#FFFFFF] text-glow text-lg" : u.rank <= 3 ? "text-[#FFFFFF]" : "text-[#888]"}`}>#{u.rank}</div>
                <div>
                  <div className="font-medium">{u.name}</div>
                  <div className="text-xs text-[#888] mono">Level {u.level}</div>
                </div>
              </div>
              <div className="mono font-semibold text-[#FFFFFF]">{u.xp.toLocaleString()} XP</div>
            </div>
          ))}
        </div>
      </div>
